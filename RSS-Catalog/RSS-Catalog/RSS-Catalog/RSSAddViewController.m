...
- (IBAction)addAction:(id)sender {
    ...
    if (_feedConnection) [_feedConnection cancel];
...
- (void) fetchURL:(NSString *) url withState:(BOOL) urlState {
    ...
    _feedConnection = [[NSURLConnection alloc] initWithRequest:rssURLRequest delegate:self];
    NSAssert(_feedConnection != nil, @"Could not create xmlConnection");
}
...
- (void) findFeedURL {
    ...
    if ([_xmlData length] < minPageSize) {
        [self errorAlert:@"This page is empty."];
        return;
    }
    ...
    if (rssLink) {
        ...
        [self errorAlert:@"I did not find a feed."];
    }
}
