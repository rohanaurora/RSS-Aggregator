...
- (void) prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"ToWebView"]) {
        RSSDetailViewController * detailViewController = (RSSDetailViewController *) [[segue destinationViewController] topViewController];

        // setup context
        NSIndexPath * path = [self.tableView indexPathForSelectedRow];
        self.itemRecord = [self.rssDB getItemRow:self.itemRowIDs[path.row]];
        detailViewController.detailItem = self.itemRecord[kItemUrlKey];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell.detailTextLabel setText:[self dateToLocalizedString:[self SQLDateToDate:thisFeedItem[kDBItemPubDateKey]]]];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    ...
    if ([error code] == kCFURLErrorNotConnectedToInternet) {
        ...
        @{NSLocalizedDescriptionKey: NSLocalizedString(@"No Connection Error", @"I don't seem to be connected to the Internet.")};
